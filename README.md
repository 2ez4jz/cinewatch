...
系统每小时扫描未来 180 天；当任一影院首次出现新的 IMAX 70mm 放映日期时，会立即发邮件到 `museycyk@gmail.com`，内容包括影院、日期、电影、场次时间和网站入口。同一天增加新场次不会重复提醒，旧日期也不会在启用功能时集中补发。

在仓库 `Settings → Secrets and variables → Actions` 中添加：

- `GMAIL_APP_PASSWORD`：`museycyk@gmail.com` 开启两步验证后生成的 16 位 Google 应用专用密码

如果邮件临时发送失败，该日期不会被标记为已通知，下一轮会再次尝试；推送失败也不会让整个座位刷新任务报错。

网页端把鼠标停在任一有座位数据的场次上，会显示即时座位缩略图；点击后仍可打开完整座位图和购票入口。

系统也会比较每次刷新前后的座位状态。如果影厅后半区原本已占的座位重新开放，并形成至少两个连续座位，会立即发送退票提醒。首次启用时只建立座位基准，不会把已有空位误报为退票。

## 刷新机制

GitHub Actions 约每 5 分钟运行一次 quick scan：检查未来 14 天，以及此前已经发现的所有远期日期。这样已知远期场次的座位也会持续刷新。

每小时执行一次 deep scan，逐日检查未来 180 天，用来发现刚开放销售的远期场次。也可以在 Actions 页面手动选择 `deep` 运行。
